...
import { calculateScore, applyEndGameBonuses, getPenaltyScore } from "../systems/scoreSystem";
...
export default function GameplayScreen({ route, navigation }) {
  ...
  const MAX_TIME = useMemo(() => {
    if (gameMode === "daily_challenge" || gameMode === "weekly_challenge") return 15;
  ...
  const [lives, setLives] = useState(
    gameMode === "lives" || gameMode === "daily_challenge" || gameMode === "weekly_challenge" 
      ? INITIAL_LIVES 
      : null
  );
  ...
  // ─── Generate soal ───────────────────────────────────────────────────────────
  const loadQuestion = useCallback((qNumOverride = null) => {
    try {
      const isChallenge = gameMode === "daily_challenge" || gameMode === "weekly_challenge";
      const qNum = (gameMode === "20_questions" || isChallenge) ? (qNumOverride || questionNumber) : null;
    ...
    async (finalScore) => {
      ...
      try {
        ...
        if (gameMode === "daily_challenge" || gameMode === "weekly_challenge") {
           const challengeType = gameMode === "daily_challenge" ? "daily" : "weekly";
           const challengeRef = doc(db, `challenge_leaderboards/${challengeType}/users`, auth.currentUser.uid);
           ...
           if (challengeSnap.exists()) {
             ...
             if (chData.score && chData.score > bonusScore) chScore = chData.score;
             if (chData.streak && chData.streak > highestStreak) chStreak = chData.streak;
           }
  ...
  const endGame = useCallback(
    async (finalScore, isTimeOut = false, isOutOfLives = false, isWin = false) => {
      if (gameOverRef.current) return; // jangan panggil dua kali
      setGameOver(true);
      await saveScore(finalScore);
      let reason = isTimeOut ? "time-up" : "wrong-answer";
      if ((gameMode === "lives" || gameMode === "daily_challenge" || gameMode === "weekly_challenge") && isOutOfLives) {
        reason = "out-of-lives";
      }
      if (isWin) {
        reason = "win";
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
      if (gameMode === "lives" || gameMode === "daily_challenge" || gameMode === "weekly_challenge") {
        setLives((prev) => {
  ...
  const handleAnswer = useCallback(
    ...
    [currentQ, combo, timeLeft, loadQuestion, endGame, gameMode, highestStreak, MAX_TIME, levelId, questionNumber],
  );
  ...
  const handleRetry = useCallback(() => {
    ...
    setTimeLeft(MAX_TIME);
    if (gameMode === "lives" || gameMode === "daily_challenge" || gameMode === "weekly_challenge") setLives(INITIAL_LIVES);
      ...
      >
        ...
        <View style={styles.topBar}>
          <View style={styles.statsBox}>
            ...
            {(gameMode === "lives" || gameMode === "daily_challenge" || gameMode === "weekly_challenge") && lives !== null && (
            ...
            {(gameMode === "20_questions" || gameMode === "daily_challenge") && (
              <Text style={{color: '#FFD700', fontWeight: 'bold', marginTop: 4, fontSize: 16}}>🎯 SOAL: {questionNumber}/20</Text>
            )}
            {gameMode === "weekly_challenge" && (
              <Text style={{color: '#FFD700', fontWeight: 'bold', marginTop: 4, fontSize: 16}}>🔥 SOAL: {questionNumber}/100</Text>
            )}
            {combo > 1 && (
              <Text style={styles.comboText}>🔥 COMBO x{combo} 🔥</Text>
            )}
            {isBossLevel(levelId) && (
               <Text style={[styles.comboText, {color: '#8e44ad'}]}>👾 BOSS LEVEL 👾</Text>
            )}
          </View>
          <View style={styles.suddenDeathBadge}>
            <Text style={styles.suddenDeathText}>
              {gameMode === "daily_challenge" ? "📅 HARIAN" : 
               gameMode === "weekly_challenge" ? "📆 MINGGUAN" :
               gameMode === "lives" ? "❤️ MODE NYAWA" : 
               gameMode === "20_questions" ? "🎯 20 SOAL" : "☠️ SUDDEN DEATH"}
            </Text>
          </View>
        </View>
